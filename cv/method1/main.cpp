...
void imageRetrival(cv::flann::Index& m_index, const string query_image_name ,
   ...
   // cout<<forSift<<endl;
    m_index.knnSearch(forAll, indices, dists, 5 );
...
int main(){
  ...
  // colorHist(matTotalColor,vec_Desc_Imgfile);
    FileStorage fs("./food/Test.xml", FileStorage::READ);
    ...
    for(;;){
        ...
        if(queNum==-1)
            break;
        forAll=matTotalDesc.row(queNum);
